...
Caz VanDevere, Hanson Shi, & Ana Chkheidze
</p>
...
The next question we ask is that is there any difference for states that expanded ACA verse those states that did not expand? The next plot shows that people living in the states that expanded ACA experienced more negative mental health than those living in the states that didn't expand ACA. However, after year 2014, there is a more significant increase for states that did not expand ACA.
...
```{r}
cut=median(brfssdata2$mental)
brfssdata2$mental_bi=ifelse(brfssdata2$mental>cut,                        1,0)
```

```{r}
reg1=glm(as.numeric(mental_bi)~year,data=brfssdata2,family="binomial") #starting with a simple model, adding elements and tidying
...
```{r}
reg2=glm(as.numeric(mental_bi)~year+ex2014,data=brfssdata2, family="binomial")
...
```{r}
reg3=glm(as.numeric(mental_bi)~year+ex2014+sex,data=brfssdata2, family="binomial")
...
```{r}
reg4=glm(as.numeric(mental_bi)~year+ex2014+sex+job,data=brfssdata2, family="binomial")
