...
title1="Average Reported Negative Mental Health Days Per Year"

ggplot(dfmelt2, aes(x=year, y = mentavg2)) +
  labs(title=title1, x=NULL, y=NULL,caption=source1) + 
  geom_line(aes(color="Average Days")) +
...
ggplot(dfsex, aes(x=year, y = mentavg, group = sex)) +
  ...
  theme(panel.background = element_rect(fill = "white",colour = "grey50"),
                              plot.caption = element_text(hjust = 0), # default was 1
                              plot.title = element_text(hjust = 0.5),
                              axis.text.x = element_text(angle = 30, hjust = 1)) + facet_grid(ex2014~.) #gender? Income bracket? Income bracket?
```
