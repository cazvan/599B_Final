...
Our dataset is comprised twonsets merged together. The first, Behavioral Risk Factor Surveillance System (BRFSS) surveys from 2000 to 2017 were created by the U.S. Center for Disease Control (CDC) and downloaded from their website: https://www.cdc.gov/brfss/annual_data/annual_data.htm. We used the following variables: mental, stab, ex2014, ex2016 sex, and year. 
...
```{r}
reg4=lm(as.numeric(mental)~year+ex2014+sex,data=brfssdata2)
summary(reg4)
```

```{r}
regstab=lm(as.numeric(mental)~year+ex2014+sex+stab,data=brfssdata2)
summary(regstab)
```

```{r}
# regression1 = tidy(reg4) %>%
    # mutate(model = "reg4") 
# summary(regression1)
```
...
```{r}
# regression2=tidy(regstab) %>% mutate(model = "regstab")
# summary(regression2)
```


```{r}
bothmodels=rbind(regstab,reg4)
dwplot(bothmodels)
